...
title: '树'
date: 2026-04-18
description: '整理普通树、二叉树与树结构遍历相关的基础概念和实现思路。'
tags: ['数据结构', '树']
categories: '数据结构'
permalink: posts/data-structure/tree/
cover: /images/notes/tree.svg
---
# 树
...
树这一章跨度很大：普通树描述层级关系，二叉树限制每个节点最多两个孩子，二叉搜索树（BST）额外要求左子树键值较小、右子树键值较大，AVL/红黑树则在搜索树上增加平衡约束。它们不是互相替代的“几个名字”，而是在同一结构上逐步增加要求。

| 结构 | 核心约束 | 典型用途 / 代价 |
| --- | --- | --- |
| 普通树 | 节点组织成父子层级 | 文件目录、组织关系；不天然支持按键快速查找 |
| 二叉树 | 每个节点最多两个孩子 | 表达式树、递归分治；不一定有序 |
| 二叉搜索树 | 左小右大（重复键规则需另定） | 平均操作 O(log n)，但退化时最坏 O(n) |
| AVL 树 | 每个节点左右子树高度差受限 | 查找较稳定，插入/删除可能触发旋转 |
| 红黑树 | 用颜色与路径规则限制高度 | 操作保证 O(log n)，常用于有序关联容器 |
| B 树 | 节点可有多个键和多个孩子 | 降低磁盘/页访问次数，常见于存储系统 |

复杂度结论背后总有前提。例如普通 BST 只有在形状较平衡时，查找才接近 O(log n)；按递增顺序插入时，它可能像链表一样高。平衡树通过维护额外不变量避免这种退化，代价是实现和更新过程更复杂。

## 用一组小数据手推遍历

考虑这棵树：根为 8，左孩子为 3、右孩子为 10；3 的孩子为 1 和 6；6 的孩子为 4 和 7；10 的右孩子为 14。
...
- 前序（根、左、右）：`8 3 1 6 4 7 10 14`
- 中序（左、根、右）：`1 3 4 6 7 8 10 14`
- 后序（左、右、根）：`1 4 7 6 3 14 10 8`
- 层序（逐层从左到右）：`8 3 10 1 6 14 4 7`

中序遍历 BST 会得到有序序列，因此它既是遍历方法，也能帮助检查搜索树性质。层序遍历使用队列；前序/中序/后序递归本质上借助了调用栈。把数据结构与算法联系起来，比单独背四个遍历名字更容易迁移到实际问题。

## 搜索树必须先定义重复值策略

“左边小、右边大”没有说明重复键放哪里。实现前需要明确一种约定：不允许重复、相等键统一放一侧，或每个节点存储一个键对应的计数/值集合。插入、查找、删除和验证函数必须共同遵守同一个约定。否则代码可能在普通样例上正确，却在重复数据时出现遗漏。

删除 BST 节点也可以按孩子数量分情况理解：叶子节点直接摘除；只有一个孩子时用孩子接上；有两个孩子时，找右子树最小节点（或左子树最大节点）替换，再删除那个后继/前驱节点。第三种情况看起来绕，是因为要同时维持“每个键的位置仍满足顺序”这个不变量。写完后应测试删除根、删除叶子、删除只有一个孩子的节点和删除有两个孩子的节点。

## 平衡树：先看不变量，再看旋转代码

AVL 树关注节点左右子树高度差；插入后若差值超出允许范围，根据失衡发生在左左、右右、左右、右左哪条路径，进行单旋或双旋。红黑树不追求每个节点都高度平衡，而是通过根为黑、红节点不能相邻、从节点到叶子路径黑节点数相同等规则限制整体高度。旋转不是“魔法修补”，它改变局部连接关系，但必须保持中序次序不变。

调试旋转时，画出旋转前后的三个关键节点和子树，再验证两件事：一是所有键仍满足搜索顺序，二是目标平衡条件得到恢复。直接对着几十行指针赋值看，很容易把“哪个节点接到哪个父节点”绕晕。树节点的父指针、根指针和空节点约定，也要在每种分支上保持一致。

## B 树与 B+ 树不要混为一谈

B 树允许一个节点保存多个键，分支数随阶数增长，因此树高比二叉树小；对外存储而言，减少层数能降低随机磁盘页读取。B+ 树通常把记录或记录指针集中放在叶子节点，内部节点主要用于导航，叶子之间常有链式连接，便于范围扫描。两者相关但并不相同；讨论具体数据库或文件系统时，要看实际采用的变体和实现。

## 学习检查清单

1. 不看笔记画出前序、中序、后序和层序，并说明其中哪些需要队列/递归栈。
2. 构造一棵退化 BST，解释为什么查找会从 O(log n) 变成 O(n)。
3. 给 BST 删除操作设计覆盖三种孩子情况的测试。
4. 画出 AVL 左左失衡的一次右旋，检查中序序列是否改变。
5. 用自己的话说明红黑树为什么能保证高度为 O(log n)，而不是只背性质列表。
6. 解释数据库使用多路树降低高度的直觉，并区分 B 树和 B+ 树。

树的代码看起来复杂，通常是因为“结构连接”和“维护性质”同时发生。把它们拆开：先画结构、再写遍历、再做普通 BST 操作，最后加入平衡规则。每一步都验证不变量，复杂的旋转和删除才会变得可追踪。
